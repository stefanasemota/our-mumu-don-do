...
export async function getTopicById(
  ...
  // For now, let's assume we can't fetch from DB on the server and use local data.
  // This is a temporary workaround until server-side Firebase is set up.

  const { firestore } = initializeFirebase(); // THIS WILL FAIL ON SERVER
